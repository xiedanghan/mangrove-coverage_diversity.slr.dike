...
if year~=1 && sum(sum(Sum_area_mark))~=0 
    row_new            = [];
    col_new            = [];
    M_mark             = find(Sum_area_mark>0); 
    [row_M,col_M]      = ind2sub(size(Sum_area_mark),M_mark); 
    for i = 1:size(M_mark,1)
        ...
        elseif sum(trv_trd(tr_mark,12)) < num0
            Growth_temp                      = []; 
            Growth_temp(1:size(tr_mark,1),:) = trv_trd(tr_mark,:); 
            trv_trd(tr_mark,:) = []; 
            f_colonize_new
            trv_trd            = [trv_trd; Growth_temp];
        end
    end
    trv_trd           = sortrows(trv_trd,1:2); 
    trv_trd(:,20)     = 1:1:size(trv_trd,1); 
