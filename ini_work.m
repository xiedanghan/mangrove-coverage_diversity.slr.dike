...
if Restart==0
    ...
    % Read info from original mdf-file fine & coarse grid
    ...
    % Note:
    % eco_timestep is the amount of minutes every ecological year
    % eco_timestep/morfac, transfer the mor minutes to hydro minutes to limit the hydro simulation time
    ...
    mdf('write',MDF1, strcat(ID1), strcat(directory,'work\')); % Update the 'Tstart' and 'Tstop'
    %     mdf('write',MDF, strcat(ID), strcat(directory,'work\'));
    
    % First model run if not already imported trim-file from restart
    % Run model
