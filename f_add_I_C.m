...
for nv = 1:num_veg_types
    if ismember(nv,trv_trd_addIC(:,11))
        ...
        out_node        = nv_tr_Loc(find(P(nv_P_Loc)<xL(nv) | P(nv_P_Loc)>xR(nv))); % find the relative hydroperiod value out of the vegetation scale
...
for iii = 1:size(IC_rc_temp,1) % Loop over vegetation cells
    ...
    clear Bio_total Competition_stress
end
...
clear Bio_total iii cell_empty