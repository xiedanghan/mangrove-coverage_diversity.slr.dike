...
if year==1 && ets==1 && Restart == 0
    fprintf(['Vegetation at Year' num2str(year) ',ETS ' num2str(ets) ' can only colonize without grow!' ]);
elseif ~isempty(trv_trd) % 09/10/2018 exclude null trv_trd
    ...
    for nv=1:num_veg_types
        ...
        D_D_T    = G(nv)*D_prev.*(1-D_prev.*H_prev/(Dmax(nv)*Hmax(nv))).*IC_prev./((274+3*b2(nv)*D_prev-4*b3(nv)*D_prev.*D_prev)*12); % diameter increasement (cm) per ets
        D_cur    = D_prev + D_D_T;
        H_cur    = 137+b2(nv)*D_cur-b3(nv)*D_cur.^2;
...
clear trv_trd_addIC vegtype H_prev IC_prev D_D_T D_cur H_cur